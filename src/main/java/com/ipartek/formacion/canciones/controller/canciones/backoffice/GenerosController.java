...
public class GenerosController extends HttpServlet {
	...
	private static final String VIEW_INDEX = "generos/index.jsp";
	private static final String VIEW_FORM = "generos/form.jsp";

	@Override
	public void init(ServletConfig config) throws ServletException {
		super.init(config);
		dao = GenerosDAO.getInstance();
	}
	...
	private void crearModificar(HttpServletRequest request) {
		...
		if (id == -1) {
			...
			if (dao.update(genero, id)) {
				...
				alert = new Alert(Alert.TIPO_DANGER, "No se puede Modificar el registro");
				request.setAttribute("genero", genero);
				view = VIEW_FORM;
			}
		}
	...
	private void eliminar(HttpServletRequest request) {
		...
		if (dao.delete(id)) {
			alert = new Alert(Alert.TIPO_SUCCESS, "Registro (" + id + ") eliminado con exito ");
		} else {
